...
public:
    ...
    TrafficSim(const TrafficSim& orig) = delete; // means the programmer doesn't want to make this
    void addCar() {
        vehicles.push_back(new Car(55, 1200, "red"));
    }
    void addBus() {
        vehicles.push_back(new Bus(40, 1600, 35));
    }
    void addTruck() {
        vehicles.push_back(new Truck(20, 5000, 8));
    }
    void payToll() {
        for (int i = 0; i < n; i++) {
            vehicles[i]->payToll();
